...
{
  [TypeLibType(4160)]
  [Guid("58D3525D-6610-4370-834B-488255ECC11C")]
  [ComImport]
  public interface IIberFuncs5 : IIberFuncs4
  {
    [DispId(1)]
    ...
    [DispId(42)]
    [MethodImpl(MethodImplOptions.InternalCall, MethodCodeType = MethodCodeType.Runtime)]
    void LogDeboutMessage([MarshalAs(UnmanagedType.BStr), In] string Text);

    [DispId(43)]
    [MethodImpl(MethodImplOptions.InternalCall, MethodCodeType = MethodCodeType.Runtime)]
    void GetSeatTotal([In] int EmpId, [In] int TableId, [In] int SeatNum, out double Subtotal, out double Tax);

    [DispId(44)]
    [MethodImpl(MethodImplOptions.InternalCall, MethodCodeType = MethodCodeType.Runtime)]
    int IsPivotSeating([In] int EmpId, [In] int RevId);
  }
}
